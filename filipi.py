...
class ImageBrowserApp:
    def __init__(self, root):
        ...
        browser_button = tk.Button(root, text="Selecionar Diretório", command=self.browse_directory)
    ...
    def browse_directory(self):
        ...
        if directory_path:
            ...
            if self.image_paths:
                ...
                self.prev_button.config(state=tk.NORMAL if len(self.image_paths) > 1 else tk.DISABLED)
                self.next_button.config(state=tk.NORMAL if len(self.image_paths) > 1 else tk.DISABLED)
            else:
